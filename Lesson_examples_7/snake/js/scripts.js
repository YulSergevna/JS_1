...
let blockTimer;  //Таймер препятствий
let blockCreationSpeed = 5000;   //Скорость появления препятствий
let numberOfBlock = [15, 0];  //Количество препятствий. 0 элемент-макс количество, 1 элемент - количество на поле
...
function moveSnake() {
    if (gameIsRunning == true) {
        ...
        if (!isSnakeUnit(newUnit) && newUnit != undefined && !isWall(newUnit)) {
...
function isWall(unit) {
    return unit.classList.contains("block-unit");
}
...
function startGame() {
    ...
    blockTimer = setInterval(createBlock, blockCreationSpeed);
}
...
function createBlock() {
    if (numberOfBlock[1] < numberOfBlock[0]) {
...
/**
 * Функция удаления препятствия на поле
 */
